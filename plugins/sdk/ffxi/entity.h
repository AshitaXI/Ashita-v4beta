...
namespace Ashita::FFXI
{
    ...
    struct look_t
    {
        ...
        uint8_t         unknown0012[14];        // Unknown. [Part of the entity look block based on the entity creation setup.]
    };
    ...
    struct entity_t
    {
        ...
        uintptr_t       ActorPointer;           // The entities actor pointer. [Type: CXiSkeletonActor* (Or other actor base types.)]
        uintptr_t       Attachments[12];        // The entities attachments. [ie. CXiSkeletonActor - Used with things such as GEO Indi's.]
        uintptr_t       EventPointer;           // The entities event pointer. [Type: XiEvent*]
        ...
        uint8_t         unknown00ED;            // Unknown. [Used to be pet mp percent, not used anymore.]
        ...
        uint8_t         unknown00F6;            // Unknown. [Set based on entity model information. Used for certain emote lookups.]
        uint8_t         padding00F7;            // Padding.
        ...
        uint32_t        unknown0178;            // Unknown. [PS2: Potentially ActorStatus]
        uint32_t        unknown017C;            // Unknown. [PS2: Potentially CliStatus]
        ...
        uint8_t         unknown01BD;            // Unknown. [Animation / emote related.]
        ...
        uint32_t        unknown01CC;            // Unknown. [Emotion / mount counter. Used for fading the character in/out.]
        ...
        uint32_t        MogMotionId;            // The current motion id of the moogle entity. [Only used for moogles.]
        ...
        uint16_t        unknown0202;            // Unknown. [Used with Attachments[1] as a timer when the entity uses an Indi spell.]
        ...
        uint32_t        unknown02A8;            // Unknown. [Set to 0 when an entity is created.]
    };
    
    static_assert(sizeof(position_t) == 32, "Invalid 'position_t' structure size detected!");
    static_assert(sizeof(move_t) == 48, "Invalid 'move_t' structure size detected!");
    static_assert(sizeof(movement_t) == 112, "Invalid 'movement_t' structure size detected!");
    static_assert(sizeof(look_t) == 32, "Invalid 'look_t' structure size detected!");
    static_assert(sizeof(render_t) == 36, "Invalid 'render_t' structure size detected!");
    static_assert(sizeof(entity_t) == 684, "Invalid 'entity_t' structure size detected!");
